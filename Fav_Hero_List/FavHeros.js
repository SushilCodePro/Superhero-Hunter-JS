let favHeroStore = getFavs();
console.log(favHeroStore.length);

let favListName = document.getElementById('fav-list-ul');
let heading = document.getElementById('heading');

if(favHeroStore.length == 0){ // if list is empty then show the empty list message
...
for(let i = 0 ; i < favHeroStore.length ; i++)
    {
        // name.innerHTML = favHeros[i];

        showDetails(favHeroStore[i]);
...
function getFavs(){ 
    let favs;
    if(localStorage.getItem('favHeroStore') === null){
      favs = [];
    }
    else{
      favs = JSON.parse(localStorage.getItem('favHeroStore'));
    }
    return favs; 
}
...
async function removeFromFavourites(e){ 
    ...
    localStorage.setItem('favHeroStore', JSON.stringify(updatedFavs));
